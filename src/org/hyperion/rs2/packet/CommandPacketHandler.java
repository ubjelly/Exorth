	...
	//	player.getUpdateFlags().flag(UpdateFlag.CHAT);
		try {
			if(command.equals("tele")) {
				...
				WorldObject obj = new WorldObject(Integer.parseInt(args[1]), player.getLocation().getX() - 2, player.getLocation().getY() - 2);
				player.getActionSender().sendWorldObjectCreation(obj, false);
			} else if(command.equals("npc")) {
				World.getWorld().register(new npc(Integer.valueOf(args[1]), player.getLocation()));
				...
				player.getActionSender().sendMessage("Entity Count: "+r.getPlayers().size()+" NPC: "+r.getNpcs().size());
					...
					for(Player p: World.getWorld().getPlayers()) {
						p.getActionSender().sendMessage(player.getName() + ": " + message);
					}
				...
				Bank.open(player);
			} else if(command.equals("giverights")) {
