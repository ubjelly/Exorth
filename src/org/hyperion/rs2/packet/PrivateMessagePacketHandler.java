...
public class PrivateMessagePacketHandler implements PacketHandler {
	...
	public static final int ADD_FRIEND = 188, MESSAGE = 126, REMOVE_FRIEND = 215;
	...
	public void handle(Player player, Packet packet) {
		...
		case REMOVE_FRIEND:
			long remove = packet.getLong();
			player.getFriends().remove(remove);
			if(Constants.CONNNECT_TO_LOGIN_SERVER) {
				World.getWorld().getLoginServerConnector().sendRemoveUser(player, remove, true);
			} else {
				if(player.getSettings().getPrivateChatSetting()[1] == 1) {
					for(Player p : World.getWorld().getPlayers()) {
						if(p.getNameAsLong() == remove) {
