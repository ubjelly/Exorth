...
public class PlayerUpdateTask implements Task {
	...
	public void execute(GameEngine context) {
		 ...
		 */
		for(Player otherPlayer : RegionManager.getLocalPlayers(player.getLocation())) {
			/*
    ...
    private static void appendHit2Update(final Player p, final PacketBuilder updateBlock) {
    	...
    	updateBlock.put((byte) p.getHealth());
    ...
    private static void appendHitUpdate(final Player p, final PacketBuilder updateBlock) {
    	...
    	updateBlock.putByteC(p.getHealth());
	...
	public void updatePlayer(PacketBuilder packet, Player otherPlayer, boolean forceAppearance) {
		...
		synchronized(otherPlayer) {
			...
			if(flags.get(UpdateFlag.FORCED_CHAT)) {
				block.putRS2String(otherPlayer.getForcedText());
			}
			if(flags.get(UpdateFlag.CHAT)) {
	...
	private void appendPlayerAppearanceUpdate(PacketBuilder packet, Player otherPlayer) {
		...
		playerProps.put((byte) otherPlayer.getHeadIcon()); // skull icon
		playerProps.put(otherPlayer.getSkullIcon()); // head icon
