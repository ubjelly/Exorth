...
public class ProjectileManager {
	...
	public static void createProjectile(Entity caster, Entity target, Projectile projectile) {
		...
		int offset = caster instanceof NPC ? ((NPC)caster).offset() : caster.getSize();
		for(Player player : RegionManager.getLocalPlayers(target.getLocation()))
			player.getActionSender().sendProjectile(caster.getLocation(), target.getLocation(), target, projectile.getGfx(), projectile.getHeight()[0], projectile.getHeight()[1], projectile.getDelay(), projectile.getSlowness(), projectile.getCurve(), offset);
	}
}
