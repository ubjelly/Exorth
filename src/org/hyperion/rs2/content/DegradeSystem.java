...
/**
 * Degradable Item System.
 * 
 * Degrades - Follow chart
 * 15 hours - 15 * 60 * 60 * 1000 / WeaponSpeed.(Default 3000)
 * 
 * @author phil
 *
 */
public class DegradeSystem {
	 ...
	 */
	public static void checkDegrade(Player player, boolean weapon) {
		...
		for(Item i : player.getEquipment().toArray()) {
			if(i != null) {
				...
				if(i.getDegrade() == 1 && weapon && i.getDefinition().getEquipmentType() == EquipmentType.WEAPON
						|| i.getDegrade() == 1 && !weapon && i.getDefinition().getEquipmentType() != EquipmentType.WEAPON) {
