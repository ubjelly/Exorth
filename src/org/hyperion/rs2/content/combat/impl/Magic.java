...
public class Magic {
	...
	public static void execute(final Entity caster, final Entity opp) {
		...
		if(sp == null && caster instanceof Player || s == null && caster instanceof NPC
		...
		double splatDelay = proj == null ? 0 : (proj.getDelay() + proj.getSlowness()) + (caster.getLocation().getDistanceFromLocation(opp.getLocation())-1) * 5;
		...
		World.getWorld().submit(new Tick(1) {//Next tick send effects.
			...
			public void execute() {
				...
				if(tick++ == 0) {
					...
					opp.playAnimation(Animation.create(st == null ? ((Player)caster).getDefEmote() : s.getDefAnim()));//Supposed to do it before the hit.
