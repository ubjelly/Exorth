...
public class Melee {
	...
	public static void execute(final Entity atker, final Entity victim) {
		...
		//end combat if NPC has no attack.
		...
		for(int i = 0; i < hitCount; i++) {
			...
			//Get base hit.
			int hit = successfull(atker, victim, specOn) ? altarDamage(atker, victim, CombatCheck.random(max)) : 0;
			...
			World.getWorld().submit(new Tick(1) {
				...
				protected void execute() {
					...
					if(victim.getHealth() <= 0 || victim.isTeleporting()) {
						this.stop();
						CombatCheck.endCombat(atker);
						return;
					}
	...
	public static boolean successfull(Entity e, Entity o, boolean s) {
		...
		if(e instanceof Player) {
			...
			effectiveAccuracy = (int) Math.floor(boost * effectiveAccuracy) + 8 + bonus;
		...
		if(o instanceof Player) {
			...
			effectiveDefence = (int) Math.floor(boost * effectiveDefence) + 8 + bonus;
	...
	public static int altarDamage(Entity e, Entity o, int hit) {
		...
		if(e instanceof Player && o instanceof Player && ((Player)o).getPrayers()[Prayer.PROTECT_FROM_MELEE])
			return (int) (hit * 0.40);
		return hit;
	}
