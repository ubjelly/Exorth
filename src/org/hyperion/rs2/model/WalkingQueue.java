...
public class WalkingQueue {
	...
	public void processNextMovement() {
		 ...
		 * The Running Energy.
		 */
		if (entity instanceof Player && !Constants.UNLIMITED_RUNNING && !teleporting) {
			Player p = (Player) entity;
            if (runEnergy < 10 && (runQueue || runToggled)) {
                runToggled = false;
                runQueue = false;
                p.getActionSender().sendConfig(173, 0);
            } else if (runQueue || runToggled) {
		 ...
		 * Reduce runEnergy.
		 */
		if (entity instanceof Player && !Constants.UNLIMITED_RUNNING && runPoint != null && !teleporting) {
			Player p = (Player) entity;
            if (runEnergy > 99 && runToggled) {
            	int weight = p.getEquipment().getWeight() + (p.getInventory().getWeight() < 0 ? 0 : p.getInventory().getWeight());
	            if(weight < 0)
	            	weight = 0;
            	runEnergy = (short) (runEnergy - (70 + (weight * 1.37)));//To be altered.
	            p.getActionSender().sendEnergy(runEnergy);
	        }
		}
		/*
		 ...
		 */
		entity.setMapRegionChanging(changed);
	}
	...
	public void setRunEnergy(int runEnergy) {
		if(runEnergy > 10000)
			runEnergy = 10000;
		this.runEnergy = (short) runEnergy;
	}
