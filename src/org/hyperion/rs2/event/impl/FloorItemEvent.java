...
public class FloorItemEvent extends Event {
	...
	public static void addFloorItem(FloorItem floor) {
		...
		Player droppedFor = floor.getDroppedFor();
			synchronized(global) {//edit, but only when able to.
				for(FloorItem i : global) {
					if(i.getLoc().getX() == floor.getLoc().getX() && i.getLoc().getY() == floor.getLoc().getY() && i.getItem() == floor.getItem()
							&& i.getLoc().getZ() == floor.getLoc().getZ() && Definitions.forId(i.getItem()).isStackable()
							&& i.getOwner().getClientIndex() == floor.getOwner().getClientIndex()) {
						floor.getOwner().getRegion().removeFloorItem(i);
						int old = i.getAmount();
						//droppedFor.getActionSender().sendRemoveRegionalItem(i);
						i.setAmount(i.getAmount() + floor.getAmount());
						floor.getOwner().getRegion().addFloorItem(i);
						droppedFor.getActionSender().sendNewRegionalItemAmount(old, i);
						return;
					}
				}
			}
